            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($choaces as $coach)
                            <tr class="hover:bg-slate-50 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <img src="https://ui-avatars.com/api/?name=Coach+Rian&background=eff6ff&color=1e40af"
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-[10px] text-slate-500 uppercase font-medium">{{ $coach['nama_klub'] }}
                                    </p>
                                </td>
    ...
    <div id="modal-tambah-coach" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-lg shadow-2xl border border-slate-200">
                ...
                <form
                    action="{{ url('/' . $user['nama_role'] . '/' . $user['uid'] . '/' . $coach['uid'] . '/dashboard/management-coach/create/process') }}"
                    method="POST" enctype="multipart/form-data" class="p-4 md:p-5">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 text-left">Nama Lengkap</label>
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 text-left">Email</label>
                            <input name="email" type="email"
                                class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-ksc-blue focus:border-ksc-blue block w-full p-2.5 outline-none"
                                required>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 text-left">Password</label>
                            <input name="password" type="password"
                                class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-ksc-blue focus:border-ksc-blue block w-full p-2.5 outline-none"
                                required>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 text-left">Nama Klub</label>
                            <input name="nama_klub" type="text"
                                class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-ksc-blue focus:border-ksc-blue block w-full p-2.5 outline-none">
                        </div>
    ...
    @foreach ($choaces as $coach)
        <div id="modal-edit-coach-{{ $coach['uid'] }}" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow-2xl border border-slate-200">
                    ...
                    <form
                        action="{{ url('/' . $user['nama_role'] . '/' . $user['uid'] . '/' . $coach['uid'] . '/dashboard/management-coach/edit/process') }}"
                        method="POST" enctype="multipart/form-data" class="p-4 md:p-5">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-slate-700 text-left">Nama
                                    Lengkap</label>
                                <input name="nama_lengkap" type="text"
                                    class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-ksc-blue focus:border-ksc-blue block w-full p-2.5 outline-none"
                            ...
                            <div>
                                ...
                                <input name="password" type="password"
                                    class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-ksc-blue focus:border-ksc-blue block w-full p-2.5 outline-none"
                                    value="{{ $coach['password'] }}" required>
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-slate-700 text-left">Nama Klub</label>
                                <input name="nama_klub" type="text"
                                    class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-ksc-blue focus:border-ksc-blue block w-full p-2.5 outline-none"
                                    value="{{ $coach['nama_klub'] }}" required>
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-slate-700 text-left">Alamat
                                    Lengkap</label>
                                <input name="alamat" type="text" <textarea rows="2" name="alamat"
                                    class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-ksc-blue focus:border-ksc-blue block w-full p-2.5 outline-none">{{ $coach['alamat'] }}</textarea>
        ...
        <div id="modal-hapus-coach-{{ $coach['uid'] }}" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-sm max-h-full">
                <div class="relative bg-white rounded-lg shadow-2xl border border-slate-200">
                    <div class="p-4 md:p-6 text-center text-left">
                        ...
                        <div class="flex justify-center gap-3">
                            <button data-modal-hide="modal-hapus-coach-{{ $coach['uid'] }}" type="button"
                                class="text-slate-500 bg-white hover:bg-slate-100 rounded-lg border border-slate-200 text-sm font-medium px-5 py-2.5 transition">Batal</button>
                            <form
                                action="{{ url('/' . $user['nama_role'] . '/' . $user['uid'] . '/' . $coach['uid'] . '/dashboard/management-coach/delete/process') }}"
                                method="POST">
                                @csrf
                                <button type="submit"
