...
@section('dashboard-section')
    ...
    <div id="modal-tambah-foto" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-md max-h-full mx-auto mt-20 md:mt-0">
            <div class="relative bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-300">
                ...
                <div class="p-6 border-b border-slate-50 flex justify-between items-center bg-white">
                    ...
                    <button data-modal-hide="modal-tambah-foto" type="button"
                ...
                <form class="p-6 space-y-4" method="POST"
                    action="{{ url('/' . $user['nama_role'] . '/' . $user['uid'] . '/dashboard/management-gallery/create/process') }}"
                    enctype="multipart/form-data">
                    @csrf
                    <div>
                        <label class="block mb-1.5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Pilih
                            Event</label>
                        <select name="uid_event" required
                            class="w-full bg-slate-50 border border-slate-200 text-xs font-bold rounded-xl p-3 outline-none focus:ring-2 focus:ring-slate-200 transition-all cursor-pointer">
                    ...
                    <div>
                        <label class="block mb-1.5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Media
                            File</label>
                        <div class="relative group">
                            <input name="foto_event[]" type="file" id="imageInput" accept="image/*" required multiple
                                class="absolute inset-0 w-full h-full opacity-0 z-50 cursor-pointer">

                            <div id="placeholderContainer"
                                class="w-full h-48 border-2 border-dashed border-slate-100 rounded-3xl flex flex-col items-center justify-center bg-slate-50/50 group-hover:bg-blue-50 group-hover:border-blue-200 transition-all overflow-hidden">
                                <div
                                    class="w-12 h-12 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                                    <i data-lucide="upload-cloud" class="w-6 h-6 text-blue-500"></i>
                                </div>
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Klik / Seret Foto
                                </p>
                                <p class="text-[8px] font-bold text-slate-300 uppercase mt-1">Maksimal 10 Foto sekaligus</p>
                            </div>

                            <div id="previewContainer"
                                class="hidden relative w-full h-64 bg-slate-50 border border-slate-100 rounded-3xl overflow-hidden shadow-inner">
                                <div
                                    class="absolute top-0 left-0 right-0 p-3 bg-white/80 backdrop-blur-md z-10 border-b border-slate-50 flex justify-between items-center">
                                    <span id="previewCount"
                                        class="text-[8px] font-black text-slate-900 uppercase tracking-widest">0 Foto
                                        Terpilih</span>
                                    <span
                                        class="text-[8px] font-bold text-slate-400 uppercase cursor-pointer hover:text-red-500 transition-colors"
                                        onclick="resetInput()">Reset</span>
                                </div>
                                <div id="previewGrid"
                                    class="grid grid-cols-3 gap-2 p-3 mt-10 overflow-y-auto max-h-[calc(100%-2.5rem)] scrollbar-hide">
                    ...
                    <button type="submit"
                        class="w-full py-4 bg-slate-900 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg transition-all active:scale-95 hover:bg-black">
                        Simpan ke Gallery
                    </button>
                </form>
            </div>
        ...
        <div class="relative w-full max-w-md max-h-full mx-auto mt-20 md:mt-0">
            <div class="relative bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-300">
                ...
                <div class="p-6 border-b border-slate-50 flex justify-between items-center bg-white">
                    ...
                    <button data-modal-hide="modal-edit-foto" type="button"
                        class="text-slate-400 hover:text-slate-900 transition-colors">
                        <i data-lucide="x" class="w-5 h-5"></i>
                    </button>
                </div>
                ...
                <form id="formEdit" class="p-6 space-y-4" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div>
                        <label class="block mb-1.5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Pindah
                            Ke
                            Event (Opsional)</label>
                        <select name="uid_event" id="edit_uid_event" required
                            class="w-full bg-slate-50 border border-slate-200 text-xs font-bold rounded-xl p-3 outline-none focus:ring-2 focus:ring-slate-200 transition-all cursor-pointer">
                            @foreach ($events as $event)
                                <option value="{{ $event['uid'] }}">{{ $event['nama_event'] }}</option>
                            @endforeach
                    ...
                    <div>
                        <label class="block mb-1.5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Update
                            Foto</label>
                        <div class="relative group">
                            <input name="foto_event" type="file" id="imageInputEdit" accept="image/*"
                                class="absolute inset-0 w-full h-full opacity-0 z-50 cursor-pointer">

                            <div id="previewContainerEdit"
                                class="relative w-full h-48 rounded-2xl overflow-hidden border border-slate-200 shadow-inner">
                                ...
                                <div
                                    ...
                                    <i data-lucide="refresh-cw" class="w-6 h-6 text-white mb-2"></i>
                                    <p class="text-[10px] text-white font-bold uppercase tracking-widest ml-2">Ganti Foto
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex gap-2">
                        <button type="button" data-modal-hide="modal-edit-foto"
                            class="flex-1 py-4 bg-slate-100 text-slate-500 font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-slate-200 transition-all">
                            Batal
                        </button>
                        <button type="submit"
                            class="flex-[2] py-4 bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all">
                            Update Gallery
                        </button>
                    </div>
                </form>
    ...
    <div id="modal-hapus-foto" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-sm max-h-full mx-auto mt-32 md:mt-0">
            <div
                class="relative bg-white rounded-[2.5rem] shadow-2xl p-8 text-center animate-in fade-in zoom-in duration-200">
                ...
                <p class="text-sm text-slate-500 font-medium mb-8">
                    Tindakan ini tidak dapat dibatalkan. Foto akan dihapus permanen dari server.
                </p>
                ...
                <div class="flex flex-col gap-3">
                    <form id="formDelete" method="POST">
                        @csrf
                        <button type="submit"
                            class="w-full py-4 bg-red-500 hover:bg-red-600 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-xl shadow-red-200 transition-all active:scale-95">
                            Ya, Hapus Permanen
                        </button>
                    </form>
                    <button data-modal-hide="modal-hapus-foto"
    ...
    <script>
        function openEditModal(uid, eventUid, imageSrc) {
            ...
            // Set Action URL
            form.action =
                `{{ url('/' . $user['nama_role'] . '/' . $user['uid']) }}/dashboard/management-gallery/${uid}/edit/process`;

            select.value = eventUid;
            preview.src = imageSrc;

            const modal = new Modal(document.getElementById('modal-edit-foto'));
            modal.show();
        }
        ...
        function openDeleteModal(uid) {
            ...
            form.action =
                `{{ url('/' . $user['nama_role'] . '/' . $user['uid']) }}/dashboard/management-gallery/${uid}/delete/process`;

            const modal = new Modal(document.getElementById('modal-hapus-foto'));
            modal.show();
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Preview Upload (Multiple Support)
            const imageInput = document.getElementById('imageInput');
            const previewGrid = document.getElementById('previewGrid');
            const previewContainer = document.getElementById('previewContainer');
            const placeholderContainer = document.getElementById('placeholderContainer');
                    ...
                    if (files.length > 0) {
                        ...
                        files.forEach(file => {
                            ...
                            reader.onload = (e) => {
                                ...
                                div.className =
                                    'relative aspect-square rounded-2xl overflow-hidden border border-slate-100 shadow-sm animate-in fade-in zoom-in duration-300';
                                div.innerHTML =
