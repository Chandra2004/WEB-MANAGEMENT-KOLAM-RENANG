...
@section('dashboard-section')
                                                    ...
                                                    <tr class="hover:bg-blue-50/30 transition">
                                                        ...
                                                        <td class="px-6 py-4 text-center">
                                                            <div class="flex items-center justify-center gap-2">
                                                                <button title="Terima Peserta"
                                                                    data-modal-target="modal-edit-registrasi-{{ $registration->user->uid }}"
                                                                    data-modal-toggle="modal-edit-registrasi-{{ $registration->user->uid }}"
                                                                ...
                                                                <button title="Tolak Peserta"
                                                                    data-modal-target="modal-hapus-registrasi-{{ $registration->user->uid }}"
                                                                    data-modal-toggle="modal-hapus-registrasi-{{ $registration->user->uid }}"
    ...
    @foreach ($events as $event)
        @foreach ($event->registrations as $registration)
                    ...
                    <div class="relative w-full max-w-lg max-h-full">
                        <div class="relative bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden"
                            ...
                            <div class="p-6 md:p-8">
                                ...
                                {{-- Form Update (Sesuai Skema: Update Status & Catatan) --}}
                                <form id="form-update-pembayaran" class="space-y-5 text-left">
                                    {{-- Input Hidden UID silakan ditambahkan Junior --}}
                                    ...
                                    {{-- Field Catatan (Muncul jika status ditolak) --}}
                                    <div x-show="status === 'ditolak'" x-transition class="space-y-2">
                                        ...
                                        <textarea name="catatan_admin" rows="3"
                                            ...
                                            placeholder="Contoh: Nominal transfer kurang atau bukti buram..."></textarea>
                                    </div>
            ...
            <div id="modal-edit-registrasi-{{ $registration->user->uid }}" tabindex="-1" aria-hidden="true"
                class="hidden fixed top-0 left-0 right-0 z-[70] w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full flex items-center justify-center bg-slate-900/60 backdrop-blur-sm transition-opacity">
                <div class="relative w-full max-w-lg max-h-full">
                    <div class="relative bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden"
                        x-data="{ status: 'menunggu' }">
                        ...
                        <div class="flex items-center justify-between p-4 md:p-6 border-b border-slate-100 bg-slate-50/50">
                            ...
                            <button type="button"
                                class="text-slate-400 hover:bg-slate-100 hover:text-slate-900 rounded-xl text-sm w-9 h-9 flex justify-center items-center transition"
                                data-modal-hide="modal-edit-registrasi-{{ $registration->user->uid }}">
                                <i data-lucide="x" class="w-5 h-5"></i>
                            </button>
                        </div>

                        {{-- Form (Action & Method silakan diisi Junior) --}}
                        <form id="form-edit-registrasi" class="p-6 md:p-8 text-left">
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <select name="status" x-model="status"
                                        ...
                                        <option value="ditolak">Ditolak (Tolak Berkas/Bayar)</option>
                                    </select>
                                    <p class="mt-3 text-[11px] text-slate-400 italic flex items-center gap-1">
                                        <i data-lucide="info" class="w-3 h-3"></i> Pastikan bukti pembayaran sudah sesuai
                                        sebelum
                                        menerima.
                                    </p>
                                </div>

                                {{-- Catatan Admin (Muncul jika status ditolak - Relevan dengan Skema Payments) --}}
                                <div x-show="status === 'ditolak'" x-transition class="space-y-2">
                                    <label
                                        class="block text-sm font-bold text-red-600 uppercase tracking-wider text-left">Alasan
                                        Penolakan</label>
                                    <textarea name="catatan_admin" rows="3"
                                        class="bg-red-50/30 border border-red-200 text-slate-900 text-sm rounded-xl focus:ring-2 focus:ring-red-500 block w-full p-3.5 outline-none"
                                        placeholder="Contoh: Bukti transfer tidak terbaca atau nominal tidak sesuai..."></textarea>
                                </div>
                            ...
                            {{-- Footer Aksi --}}
                            <div class="flex items-center pt-8 mt-6 border-t border-slate-100 space-x-3 justify-end">
                                <button data-modal-hide="modal-edit-registrasi-{{ $registration->user->uid }}"
                                    type="button"
                                    class="text-slate-500 bg-white hover:bg-slate-50 rounded-xl border border-slate-200 text-sm font-bold px-8 py-3 transition">Batal</button>
                                <button type="submit"
            ...
            <div id="modal-hapus-registrasi-{{ $registration->user->uid }}" tabindex="-1" aria-hidden="true"
                class="hidden fixed top-0 left-0 right-0 z-[70] w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full flex items-center justify-center bg-slate-900/60 backdrop-blur-sm transition-opacity">
                <div class="relative w-full max-w-sm max-h-full text-">
                    <div class="relative bg-white rounded-lg shadow-2xl border border-slate-200 text-">
                        <div class="p-4 md:p-6 text-center text-">
                            ...
                            <div class="flex justify-center gap-3 text-">
                                <button data-modal-hide="modal-hapus-registrasi-{{ $registration->user->uid }}"
                                    type="button"
